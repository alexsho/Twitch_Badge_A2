...
//Alex Version
//Drawbitmap() has invert parameter
...
const String keyword_7 = "pacman("; //pacman eating 
...
void parseMessage(String cmd_str){ //parses the string in differnt manner bases on the the keyword after the special character
  ...
  if(cmd_str.indexOf(keyword_1)>=0 && cmd_str.indexOf(")")>0) { //check for a keyword and the closed parenthesis 
	...
	//Pacman Eating pacman(10)
    if(cmd_str.indexOf(keyword_7)>=0 && cmd_str.indexOf(")")>0) { //check for a keyword and the closed parenthesis
	    int cmd_start = keyword_7.length();
	    int cmd_end = cmd_str.indexOf(")");
	    String buffer_str = cmd_str.substring(cmd_start,cmd_end);
	   	//Serial.print(buffer_str.c_str());
	   	unsigned int i;
		...
		Serial.printf("i=%d\n",i);
			if(!(i<0 || i>MAXPACMANEAT)){ //check that vars are in "bounds"
			lmd.clear();
			for(int k=0; k<i; k++){
				getrand();
				ColorFill(Forecolor);
				drawBitmap(bitmap2,0); // display pacman with mouth closed
			  	lmd.display();					
				delay(500);
				getrand();
				ColorFill(Forecolor);			
				drawBitmap(bitmap3,0); // display pacman with mouth semi open
			  	lmd.display();					
				delay(500);
				getrand();
				ColorFill(Forecolor);				  				
				drawBitmap(bitmap4,0); // display pacman with mouth open
			  	lmd.display();						
				delay(500);
				getrand();
				ColorFill(Forecolor);			  
			...
			delay(100);				
		  	...
		  	ColorFill(0);				
					...
					Serial.printf("%s ERROR !!!",keyword_7.c_str());
	}
